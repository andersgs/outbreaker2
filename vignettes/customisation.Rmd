...
only difference is that custom functions will take three arguments (`data`, 
`param`, and `i`) instead of one in the prior functions. The function used to specify
custom likelihood is `custom_likelihoods`. The arguments `data` and `param` are
self-explanatory, but the argument `i` is either an integer or a vector of integers. 
The `i` is used to identify the individual(s) that are current focus of the likelihood
function.

Each custom function will correspond to a specific likelihood component:
